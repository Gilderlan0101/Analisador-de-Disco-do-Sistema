...
from src.backup.config import COMMAND_REGISTRY
from src.backup.config import SCOPES
from src.backup.config import Colors
...
class OptionsInLine:
    ...
    def commands_in_line(self) -> None:
        ...
        print(f"{Colors.CYAN}{Colors.BOLD}      GOOGLE DRIVE BACKUP MANAGER{Colors.RESET}")
        ...
        for index, command in enumerate(COMMAND_REGISTRY_COPY, start=1):
            ...
            print(f"{Colors.GREEN}{index:2}. {Colors.BOLD}{cmd_name:<15}{Colors.RESET}{Colors.YELLOW}-{Colors.RESET} {cmd_desc}")
        
        print(f"\n{Colors.CYAN}{'─'*50}{Colors.RESET}")
        print(f"{Colors.MAGENTA}💡 Dica:{Colors.RESET} Use {Colors.BOLD}'init'{Colors.RESET} para autenticar pela primeira vez")
...
class RunCommands(OptionsInLine):
    ...
    async def input_field(self) -> None:
        ...
        while True:
            try:
                print(f"{Colors.BLUE}╭─({Colors.GREEN}backup-manager{Colors.BLUE})─[{Colors.WHITE}~{Colors.BLUE}]{Colors.RESET}")
                entry = str(input(f"{Colors.BLUE}╰─{Colors.BOLD}${Colors.RESET} > ")).strip()

                if entry.startswith('backup --all'):
                    print(f"\n{Colors.YELLOW}⚠️  Fazendo backup de todos os arquivos...{Colors.RESET}")
                    print(f"{Colors.YELLOW}   Isso pode remover arquivos do armazenamento local.{Colors.RESET}\n")

                elif entry.startswith('backup -f'):
                    print(f"\n{Colors.CYAN}📤 UPLOAD DE ARQUIVO{Colors.RESET}")
                    print(f"{Colors.CYAN}{'─'*30}{Colors.RESET}")
                    current_dir = os.getcwd()
                    print(f"{Colors.WHITE}Diretório atual: {Colors.GREEN}{current_dir}{Colors.RESET}")
                    
                    file_path = str(input(f"\n{Colors.WHITE}📁 Caminho do arquivo: {Colors.RESET}")).strip()
                    mime_type_file = str(input(f"{Colors.WHITE}📝 Tipo MIME (opcional): {Colors.RESET}")).strip()
                    
                    if mime_type_file == '':
                        mime_type_file = None
                    
                    print(f"\n{Colors.YELLOW}⏳ Enviando arquivo...{Colors.RESET}")
                    await Commands().upload(file_path=file_path, mime_type=mime_type_file)
                    ...
                    id_file = str(input(f"\n{Colors.WHITE}🔑 ID do arquivo para excluir: {Colors.RESET}")).strip()
                    ...
                    print(f"{Colors.YELLOW}📝 Comandos disponíveis:{Colors.RESET}")
                    self.commands_in_line()
...
class Commands(RunCommands):
    ...
    async def authorization_user(self) -> None:
        ...
        if not self.creds or not self.creds.valid:
            if self.creds and self.creds.expired and self.creds.refresh_token:
                print(f"{Colors.YELLOW}🔄 Atualizando token de acesso...{Colors.RESET}")
                self.creds.refresh(Request())
            else:
                print(f"{Colors.CYAN}🌐 Iniciando autenticação...{Colors.RESET}")
                print(f"{Colors.YELLOW}⚠️  Uma janela do navegador será aberta para autorização.{Colors.RESET}")
                flow = InstalledAppFlow.from_client_secrets_file('credentials.json', SCOPES)
                self.creds = flow.run_local_server(port=0)

            with open('token.json', 'w') as token:
                token.write(self.creds.to_json())
            print(f"{Colors.GREEN}✅ Autenticação concluída com sucesso!{Colors.RESET}")
        else:
    ...
    async def list(self) -> None:
        ...
        try:
            ...
            results = (
                service.files()
                .list(pageSize=20, fields='nextPageToken, files(id, name, size, modifiedTime)')
                .execute()
            )
            ...
            if not items:
                print(f"{Colors.YELLOW}📭 Nenhum arquivo encontrado no Google Drive.{Colors.RESET}")
                return

            print(f"\n{Colors.GREEN}📁 Total de arquivos: {len(items)}{Colors.RESET}\n")
            print(f"{Colors.CYAN}{'─'*70}{Colors.RESET}")
            print(f"{Colors.BOLD}{'NOME':<40} {'ID':<25} {'MODIFICAÇÃO':<10}{Colors.RESET}")
            print(f"{Colors.CYAN}{'─'*70}{Colors.RESET}")
            
            for item in items:
                name = item['name'][:38] + '...' if len(item['name']) > 38 else item['name']
                file_id = item['id'] if len(item['id']) > 22 else item['id']
                modified = item.get('modifiedTime', 'N/A')[:10] if 'modifiedTime' in item else 'N/A'
                print(f"{Colors.WHITE}{name:<40} {Colors.CYAN}{file_id} {Colors.YELLOW}{modified:<10}{Colors.RESET}")
            
            print(f"{Colors.CYAN}{'─'*70}{Colors.RESET}\n")

        except HttpError as error:
            print(f"{Colors.RED}❌ Erro ao listar arquivos: {error}{Colors.RESET}")
    ...
    async def delete_file(self, file_Id: str) -> None:
        ...
        try:
            ...
            print(f"{Colors.RED}   Arquivo: {Colors.BOLD}{file_name}{Colors.RESET}")
            print(f"{Colors.RED}   ID: {file_Id}{Colors.RESET}")
            
            confirm = input(f"\n{Colors.YELLOW}❓ Digite 'SIM' para confirmar: {Colors.RESET}").strip().upper()
            
            if confirm == 'SIM':
                service.files().delete(fileId=file_Id).execute()
                print(f"{Colors.GREEN}✅ Arquivo '{file_name}' excluído com sucesso!{Colors.RESET}")
            else:
                print(f"{Colors.YELLOW}⏹️  Exclusão cancelada.{Colors.RESET}")
                
        except HttpError as error:
            print(f"{Colors.RED}❌ Erro ao excluir arquivo: {error}{Colors.RESET}")
        except Exception as e:
    ...
    async def upload(self, file_path: str, mime_type: str = None) -> None:
        ...
        try:
            if not os.path.exists(file_path):
                print(f"{Colors.RED}❌ Arquivo não encontrado: {file_path}{Colors.RESET}")
                return
            ...
            print(f"{Colors.WHITE}   Nome: {Colors.GREEN}{file_name}{Colors.RESET}")
            print(f"{Colors.WHITE}   Tamanho: {Colors.GREEN}{file_size:,} bytes{Colors.RESET}")
            print(f"{Colors.WHITE}   Tipo MIME: {Colors.GREEN}{mime_type}{Colors.RESET}")
            ...
            print(f"{Colors.GREEN}✅ Upload concluído com sucesso!{Colors.RESET}")
            print(f"{Colors.CYAN}🔗 ID do arquivo: {Colors.BOLD}{file.get('id')}{Colors.RESET}\n")
